...
movie_recommendation <- function(input,input2,input3) {
  ...
  # Convert rating matrix into a more compact "realRatingMatrix"
  ratingmat2 <- as(ratingmat2, "realRatingMatrix")
  
  # User-based collaborative filtering (UBCF) with Pearson distance model
  recommender_model <- Recommender(ratingmat2, method = "UBCF",param=list(method = "pearson",nn=30))
  ...
  if (as.character(recom_list[1])=='character(0)'){
    no_result[1,1] <- "Oops, there seems to be insufficient information to recommend a movie for you. Please try again"
    ...
    colnames(recom_result) <- "... here are the recommended Movies for you"
    return(recom_result)
  }
}